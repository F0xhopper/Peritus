...
| [accounts.md](accounts.md) | Password sign-in, sign-up with code confirmation, reset by code, and Settings account management (profile, email, password, Google link, sessions, deletion) — `api/routes/account.py`, `api/accounts/`, `web/app/(auth)/`, `web/components/settings/account/`. Supabase dashboard steps are in `web/docs/deploy.md` |
...
| [beating-closed-book.md](beating-closed-book.md) | Evaluation (2026-09-19) of retrieval and answers against the complaint "no better than normal GPT": a blind head-to-head with the same model closed-book (6–4–2 against, Peritus ahead only on depth), every loss traced to a cause — long works cut at 200k characters, an absolute rerank floor that starves weak retrievals, a recovery pass that rarely runs, no seat per part of a question, no retrieval path for broad questions, a graph that retrieves nothing, no quotations or loci, leaking personas, a century-old corpus for a practical subject; a six-phase plan, each phase gated on the head-to-head. Run data in `beating-closed-book/` |
...
| [expert-brain.md](expert-brain.md) | One map per expert (2026-09-18): Sources and Concepts merged into a *Knowledge* page with Map and List views — syllabus ring, concept cloud, source orbit, missing named texts as gaps; why today's graph is dots and its bridge to sources has never rendered; the one missing join (concept → key concept) and its measured gate; the tilt-and-orbit idle motion that flattens on engagement; `/map` API; nine phases |
| [expert-brain-interactive.md](expert-brain-interactive.md) | Companion to expert-brain.md (2026-09-18): the map as the interactive expert — every change to what it knows starts and lands there (add from a gap, a key concept or a dropped file; removal that says what goes with it; later, find a missing text and extend the syllabus); and the build as the brain growing, from one reducer over the durable event log that also animates a single added source and replays finished builds; the two event fields that are missing; why concepts are not drawn before `graph_ready`; phases G0–G6 |
| [passage-reader.md](passage-reader.md) | The passage in its source (2026-09-17): why citation text is lost on reload (schema), a windowed "in context" view from the chunks already stored, a whole-source reader gated by kind, and why not to render originals with a highlight |
