...
async def _expert(pool, name: str):
    return await ExpertRepository(pool).create(name=name, topic="beekeeping", tier=ExpertTier.LITE)
...
def _extraction(chunk_id: int, claim: str, edges: list[dict]) -> dict:
    return {
        "nodes": [
            ...
            {
                "label": "Varroa destructor",
                "node_type": "concept",
                "description": "A parasitic mite",
                "chunk_db_ids": [chunk_id],
            },
        ],
        "edges": edges,
...
async def test_ingest_rejects_edges_the_endpoints_do_not_support(db_pool):
    ...
    _, edge_count = await repo.bulk_insert_from_extractions(
        expert.id,
        [
            {
                "nodes": [
                ...
                "edges": [
                    ...
                    {
                        "from_label": "Varroa destructor",
                        "to_label": "Colonies collapse",
                        "edge_type": "about",
                    },
                    # Kept.
                    {
                        "from_label": "Colonies collapse",
...
async def test_claims_by_concept_groups_across_sources(db_pool):
    ...
    about = [
        {
            "from_label": "Mites suppress immunity",
            "to_label": "Varroa destructor",
            "edge_type": "about",
        }
    ]
    await repo.bulk_insert_from_extractions(
        expert.id,
        [
            _extraction(a, "Mites suppress immunity", about),
            _extraction(
                b,
                "Mites do not suppress immunity",
                [
                    {
...
async def test_insert_relations_and_evidence_count(db_pool):
    ...
    claims = {c.label: c.node_id for g in await repo.claims_by_concept(expert.id) for c in g.claims}

    inserted = await repo.insert_relations(
        expert.id,
        [
