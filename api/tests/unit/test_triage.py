...
def _triaged(
    title: str, url: str, score: float, st: SourceType = SourceType.WEB
) -> TriagedCandidate:
...
async def test_fetch_with_refill_stops_at_budget():
    ...
    ranked = [_triaged(f"t{i}", f"https://x.test/{i}", 9 - i * 0.1) for i in range(10)]
    results, _committed = await builder._fetch_with_refill(
        ranked, budget=4, caps={SourceType.WEB: 99}
    )
...
async def test_fetch_with_refill_replaces_failures_from_lower_ranks():
    ...
    ranked = [failing] + [_triaged(f"t{i}", f"https://x.test/{i}", 8 - i * 0.1) for i in range(4)]
    results, _committed = await builder._fetch_with_refill(
        ranked, budget=3, caps={SourceType.WEB: 99}
    )
...
async def test_fetch_with_refill_enforces_per_type_caps():
    builder = _builder_with_fake_fetchers()
    ranked = [
        _triaged(f"web{i}", f"https://w.test/{i}", 9 - i * 0.1, SourceType.WEB) for i in range(5)
    ] + [_triaged(f"exa{i}", f"https://e.test/{i}", 5 - i * 0.1, SourceType.EXA) for i in range(5)]
    results, _committed = await builder._fetch_with_refill(
        ranked,
        budget=6,
        caps={SourceType.WEB: 2, SourceType.EXA: 99},
    )
