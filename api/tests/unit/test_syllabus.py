...
def _candidate(title: str, url: str = "", source_type=SourceType.GUTENBERG, **meta) -> SourceCandidate:
    return SourceCandidate(
        source_type, url or f"https://x.test/{abs(hash(title))}", title, None, "snippet",
        metadata=dict(meta),
    )
...
def test_the_volumes_of_one_work_are_all_kept():
    ...
    ranked = rank_candidates([
        _triaged(_candidate(title, must_have_title="Summa Theologiae", must_have_sections=sections))
        for title, sections in zip(_SUMMA_VOLUMES, ("I qq. 2-3", "I-II qq. 90-97", "III qq. 1-6"), strict=True)
    ])
...
def test_volumes_from_one_lookup_differ_by_designator_and_are_kept():
    ranked = rank_candidates([
        _triaged(_candidate(title, must_have_title="Summa Theologiae")) for title in _SUMMA_VOLUMES
    ])
    assert len(ranked) == 3
...
def test_two_copies_of_one_volume_from_two_routes_still_collapse():
    ranked = rank_candidates([
        _triaged(_candidate(
            "Summa Theologica, Part I-II", "https://gutenberg.org/ebooks/1",
            must_have_title="Summa Theologiae", must_have_sections="I-II qq. 90-97",
        )),
        _triaged(_candidate(
            "Summa Theologica Part I-II", "https://archive.org/details/summa",
            must_have_title="Summa Theologiae", must_have_sections="I-II qq. 90-97",
        ), 7.0),
    ])
    assert len(ranked) == 1
...
def test_later_rounds_stop_boosting_once_the_work_is_in_the_corpus():
    ...
    raw = RawSource(
        SourceType.GUTENBERG, "https://gutenberg.org/ebooks/2", "Summa Contra Gentiles", None, "x",
...
async def test_the_fetch_stage_enforces_the_ceiling_before_costing_the_source():
    ...
    sources, _ = await builder._fetch_with_refill(
        [_triaged(candidate, 9.0)], 5, {}, budget_usd=Decimal(3),
    )
...
def test_the_outline_keeps_two_levels_and_drops_the_apparatus():
    ...
    assert headings == [
        (1, "Philosophy"), (2, "Metaphysics"), (2, "Epistemology"),
        (1, "Theology"), (1, "History"), (2, "Neo-Thomism"), (2, "Analytic Thomism"),
    ]
...
def test_a_reference_entry_s_numbered_contents_is_its_outline():
    ...
    assert headings == [(1, "Life and works"), (1, "Metaphysics"), (2, "Act and potency"), (1, "Legacy")]
...
def test_the_hit_is_the_article_titled_after_the_topic():
    ...
    assert select_hit("Thomism", hits[:2])["url"] == "a", "a snippet naming the topic is the fallback"
    assert select_hit("Thomism", hits[1:2]) is None


def test_the_pack_renders_as_the_planner_reads_it():
    pack = OrientationPack([
        Overview("Wikipedia", "Thomism", "https://en.wikipedia.org/wiki/Thomism", "A school.",
...
async def test_the_planner_is_shown_the_pack_and_the_plan_records_what_was_read():
    ...
    plan, call = await _plan_with(pack, {
    ...
    assert plan["orientation"] == {
        "overviews": [{"source": "Wikipedia", "title": "Thomism", "url": "u", "headings": ["History"]}],
        "note": "",
    }
...
def test_facets_flatten_into_key_concepts_deduplicated_across_facets():
    ...
    assert plan["key_concepts"] == ["Act and potency", "Analogy of being", "Neo-Thomism", "Natural law"]
...
_STANDARD_TARGET = CoverageTarget(
    min_sources=2, min_source_types=1, require_non_tertiary=False, max_rounds=2, require_primary=True,
)


def _src(concepts: dict[str, str], tier="primary", source_type=SourceType.WEB, **meta) -> ValidatedSource:
    raw = RawSource(source_type, f"https://x.test/{abs(hash(str(concepts)))}", "T", None, "text", metadata=meta)
...
def test_figures_are_normalised_with_one_obtainable_work_or_none():
    plan = _normalise_plan({
        ...
        "figures": [
            {"name": "Edward Feser", "why": "analytic Thomism", "obtainable": True,
             "work": {"title": "Five Proofs blog essays", "kind": "text", "public_domain": False, "open_text": True}},
...
def test_the_channel_excludes_about_hosts_and_asks_for_personal_sites():
    (topic_query, topic_kwargs), (name_query, name_kwargs) = people_search_calls("Edward Feser", "Thomism")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Aquinas, Thomas | Internet Encyclopedia of Philosophy", "https://iep.utm.edu/aquinas/", True),
        ...
        ("Edward Feser: The road from atheism", "https://edwardfeser.blogspot.com/2012/the-road.html", False),
...
async def test_plan_figures_replace_the_blind_identification_call(monkeypatch):
    ...
    async def _content(person, topic):
        ...
        return [
            SourceCandidate(SourceType.THOUGHT_LEADER, f"https://{person['name'][0]}.test/{i}",
                            f"Essay {i}", person["name"], "s", metadata={"leader": person["name"]})
            for i in range(3)
        ] + [SourceCandidate(SourceType.THOUGHT_LEADER, "https://plato.stanford.edu/entries/x/",
                             "Entry", None, "s", metadata={"leader": person["name"]})]
...
def test_an_in_copyright_book_skips_the_public_domain_libraries():
    work = MustHaveWork("The Elements of Christian Philosophy", "Étienne Gilson", "book", public_domain=False)
...
def test_not_obtainable_is_reported_with_its_substitute():
    work = MustHaveWork.from_plan({
        "title": "The Elements of Christian Philosophy", "author": "Gilson", "kind": "book",
        "public_domain": False,
        "substitute": {"title": "The Spirit of Mediaeval Philosophy", "kind": "book", "public_domain": True},
    })
...
async def test_a_substitute_is_looked_for_only_when_the_work_cannot_be_had(monkeypatch):
    ...
    gilson = MustHaveWork.from_plan({
        "title": "Elements", "kind": "book", "public_domain": False,
        "substitute": {"title": "Spirit", "kind": "book", "public_domain": True},
    })
    summa = MustHaveWork.from_plan({
        "title": "Summa", "kind": "text", "public_domain": True,
...
def test_figure_statuses():
    ...
    works = [
        MustHaveWork("Commentary on De Ente", "Cajetan", "text", True, scope=SCOPE_FIGURE, figure="Cajetan"),
    ]
    passed = [
    ...
    assert statuses == {
        "Feser": "own_voice", "Maritain": "about_only", "Gilson": NOT_OBTAINABLE, "Cajetan": "not_found",
    }
...
def test_the_validator_calls_an_about_page_tertiary():
    ...
    raw = RawSource(SourceType.THOUGHT_LEADER, "u", "t", None, "x", metadata={"leader": "Jacques Maritain"})
...
def test_tags_are_matched_with_their_depth_and_legacy_strings_read_as_treats():
    ...
    assert depths == {
        "Natural law": "sets_out", "analogy": "mentions", "Act and potency": "treats", "essence": "treats",
    }
    assert covered_names(depths) == ["Natural law", "Act and potency", "essence"]


_LITE_TARGET = CoverageTarget(min_sources=1, min_source_types=1, require_non_tertiary=False, max_rounds=1)


def test_mentions_never_count_and_a_survey_counts_for_its_three_deepest():
    survey = _src({
        "a": DEPTH_TREATS, "b": DEPTH_SETS_OUT, "c": DEPTH_TREATS, "d": DEPTH_TREATS, "e": DEPTH_MENTIONS,
    })
...
def test_the_named_text_gates_has_primary(named, depth, has_primary):
    passed = [_src({"natural law": depth}), _src({"natural law": depth}, source_type=SourceType.GUTENBERG)]
    named_texts = {"natural law": named} if named else None
    [concept] = compute_coverage(["natural law"], passed, _STANDARD_TARGET, named_texts=named_texts).concepts
...
def test_found_named_text_satisfies_primary_even_with_no_primary_tag():
    passed = [_src({"x": DEPTH_TREATS}, tier="secondary"), _src({"x": DEPTH_TREATS}, tier="secondary")]
    [concept] = compute_coverage(["x"], passed, _STANDARD_TARGET, named_texts={"x": NAMED_FOUND}).concepts
...
def test_lite_is_unchanged_by_the_gate():
    ...
    [concept] = compute_coverage(
        ["natural law"], [_src({"natural law": DEPTH_TREATS})], lite, named_texts={"natural law": NAMED_MISSING},
    ).concepts
    assert concept.met
...
def test_the_named_text_is_judged_per_lookup_not_per_work():
    ...
    works = [
        MustHaveWork("Summa Theologiae", "Aquinas", "text", True, "I qq. 2-3", SCOPE_CONCEPT, ["five ways"]),
        MustHaveWork("Summa Theologiae", "Aquinas", "text", True, "I-II qq. 90-97", SCOPE_CONCEPT, ["natural law"]),
        MustHaveWork("De Ente et Essentia", "Aquinas", "text", True, "", SCOPE_CONCEPT, ["real distinction"]),
        MustHaveWork("De Veritate", "Aquinas", "text", True, "q. 1", SCOPE_CONCEPT, ["transcendentals"]),
    ]
    passed = [
        ("https://g/1", _meta(must_have_title="Summa Theologiae", must_have_extent="partial",
                              must_have_concepts=["five ways"], sections_matched=True)),
        ("https://d/1", _meta(must_have_title="De Ente et Essentia", must_have_extent=EXTENT_WHOLE)),
        ("https://v/1", _meta(must_have_title="De Veritate", must_have_extent="partial",
                              must_have_concepts=["transcendentals"])),
    ]
    named = concept_named_texts(works, passed)
    assert {c: e["status"] for c, e in named.items()} == {
        "five ways": "found", "natural law": "missing", "real distinction": "found", "transcendentals": "partial",
    }
...
def test_a_found_substitute_counts_for_the_concept():
    work = MustHaveWork.from_plan({
        "concept": "being", "title": "Elements", "kind": "book", "public_domain": False,
        "substitute": {"title": "Spirit", "kind": "book", "public_domain": True},
    }, SCOPE_CONCEPT)
...
def test_the_corpus_summary_reports_what_the_gate_sees():
    ...
    assert corpus["concepts_missing_named_text"] == [{"concept": "natural law", "texts": ["Summa I-II qq. 90-97"]}]
