 ...
 *
 * It is the Graph view's own panel, and not the Knowledge page's
 * `ConceptPanel`, because a node here is a concept **or a claim** — `about`,
 * `supports` and `contradicts` edges run to claims — and the map's concept
 * endpoint knows only concepts. A concept the Map also draws offers the way
 * across.
 */
...
export function NodeDetail({
  ...
  /** Ask the expert, in a new chat — the same action as every other panel here. */
  onAsk: (text: string) => void
  /** True while the chat that question will be asked in is being created. */
  asking?: boolean
  /** Present only for a concept the Map also draws. */
      ...
      <div className="flex flex-wrap gap-2 border-t border-border-soft pt-3">
        ...
        {keyConcepts.some((c) => c.toLowerCase() === node.label.toLowerCase()) && (
          <ButtonLink
            variant="outline"
            size="sm"
            href={`/experts/${slug}/knowledge?view=list&concept=${encodeURIComponent(node.label)}`}
          >
          ...
          onClick={() =>
            onAsk(
              node.node_type === 'claim'
                ? `What do the sources say about this claim: “${node.label}”`
                : `What do the sources say about ${node.label}?`
            )
          }
        >
