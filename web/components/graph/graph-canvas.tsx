...
export function GraphCanvas({
  ...
  /**
   * The node ids the Knowledge page has lit, or null. Must be referentially
   * stable while it means the same thing: a new Set is a new frame.
   */
  ...
  }, [nodes, edges, selectedId, lit, labelBudget])
  ...
  // What is lit, and what is selected, can change with no gesture and no tick
  // behind it — a row of the Overview under the pointer, a match chosen in the
  // search — and a settled graph paints nothing per frame. So they ask for one.
  // Declared after the effect that refreshes `drawRef`, which must run first.
  useEffect(() => {
    requestDraw()
  }, [lit, selectedId, requestDraw])
  ...
  return (
    <div ref={wrapperRef} className={cn('relative h-full w-full', className)}>
      <canvas
        ref={canvasRef}
        // A canvas is opaque to a screen reader; this is what it draws. The
        // nodes themselves are reachable through the page's search.
        role="img"
        aria-label={`Graph of ${nodes.length} concepts and claims, joined by ${edges.length} links`}
